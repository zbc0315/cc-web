...
export interface PromptCardProps {
  ...
  preview: string;
  /** Only meaningful for `agent-prompt`; controls the green status dot. */
  inserted?: boolean;
  ...
  /** When set, card looks "system" / read-only — no edit/delete shown; share still OK. */
  readOnly?: boolean;
}
...
export function PromptCard({
  kind, label, preview, inserted, onLeftClick,
  onEdit, onDelete, onShare, footer, readOnly,
}: PromptCardProps) {
      ...
      <ContextMenuTrigger asChild>
        <div
          ...
          title={
            kind === 'quick-prompt'
              ? '点击发送 · 右键更多'
              : (inserted ? '点击从 CLAUDE.md 移除 · 右键更多' : '点击插入 CLAUDE.md · 右键更多')
          }
        >
          {/* Status dot (agent-prompt only) */}
          {kind === 'agent-prompt' && (
            <span
              aria-hidden
              className={cn(
          ...
          <div className={cn('py-1.5 pr-7', kind === 'agent-prompt' ? 'pl-5' : 'pl-3')}>
          ...
          {/* Kebab: hover-reveal on desktop (hover:hover media query), always
              visible on touch so the menu is reachable without a right-click.
              `hover:opacity-100` combines with `md:opacity-0 md:group-hover:opacity-100`
              — on touch media, Tailwind's `group-hover` variant won't fire. */}
          <button
