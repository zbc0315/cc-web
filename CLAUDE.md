...
- **Security & auth hardening (v1.5.87)**: Project mutation routes (DELETE/stop/start/archive) add owner/admin/edit-share permission checks. OAuth backup `state` parameter now passed through all 3 providers (Google/OneDrive/Dropbox). Git add path traversal blocked via `path.resolve` validation. Plugin data routes validate ID format. OfficePreview XSS fixed with DOMPurify sanitization. FloatWindow postMessage uses `window.location.origin` instead of `'*'`. Plan-control `ifChainMatched` changed from single boolean to `Map<number, boolean>` for nested if-chain correctness. `removeExecutor` calls `stop()` before deletion. Dead code cleanup (chatMessages → ref, Google Drive stat).
- **Voice-to-text + PluginDock capsule (v1.5.86)**: `TerminalDraftInput` mic button (Web Speech API, `zh-CN`); long-press spacebar >300ms triggers recording. `PluginDock` redesigned as floating semi-transparent capsule with circular icon buttons.
- **Plan-Control DSL system (v1.5.84–85)**: Custom "pc language" for deterministic AI task orchestration. Backend: `plan-control/` — parser, checker, executor (state machine IDLE→SENDING→WAITING→PROCESSING, file-based AI communication via nodes/ JSON, fs.watch+polling, nudge, crash recovery). `routes/plan-control.ts` REST API with auth checks. Frontend: `PlanPanel` + `TaskTree` SVG topology. v1.5.85 added anti-pattern rules in INIT_MD template + DSL feedback channel.
- **Fix: xterm.js ReferenceError in prod (v1.5.82)**: `@xterm/xterm@6.0.0` const enum IIFE broken by esbuild minifier. Switched to terser.
- **Fix: X-Frame-Options DENY blocked plugin iframes (v1.5.83)**: Changed to `SAMEORIGIN` + `frame-src 'self'`.
- **Fix: opencode adapter infinite restart (v1.5.81)**: Removed invalid `--yolo` flag; `buildCommand()` returns plain `'opencode'`.
- **Fix: LAN file download broken (v1.5.80)**: `fetch`→`Blob`→`createObjectURL` pattern + `?dl=1` Content-Disposition header.
- **Fix: WSL2 missing config.json (v1.5.79)**: `'__local_admin__'` sentinel for localhost auth when no config exists.
- **CliToolAdapter pattern (v1.5.78)**: `backend/src/adapters/` — strategy pattern for multi-CLI support (Claude/OpenCode/Codex/Qwen). Windows platform gate. Usage monitor settings. Notifications broadcast to both dashboard and project WS clients.
- **WSL/Windows compatibility (v1.5.76)**: Shell fallback zsh→bash, adm-zip replaces unzip, pure JS ZIP builder, WSL browser detection, detached daemon fork, os.tmpdir() for multer.
- **Plugin system (v1.5.75)**: `plugin-manager.ts` lifecycle, `FloatWindow.tsx` iframe sandbox, `PluginDock`, `PluginLayer` scope filtering, Plugin SDK (postMessage RPC), CLI scaffold (`ccweb-plugin create/build`), bundled Pomodoro plugin. manifest.json schema with scope/permission constraints.
- **Terminal draft input (v1.5.59–74)**: Three-state `draftMode` (bottom/float/hidden, Ctrl+I cycle). Draggable float position persisted per project. Toolbar with Skills panel (`GET /api/claude/skills` → builtin/custom/mcp) and Model dropdown (`/model <name>`). `Shift+Enter` sends, `Enter` newline. StopCircle interrupt button. Resizable left/right panels (drag handle, localStorage persistence).
- **Non-modal floating dialogs (v1.5.67)**: IDE-style palettes via `useProjectDialogStore` (Zustand). `modal={false}` + `noOverlay` on Radix Dialog. Focus/unfocus opacity states.
- **Pomodoro timer (v1.5.57→64)**: Global architecture — `usePomodoroStore` (Zustand), `PomodoroController`/`PomodoroOverlay` in App.tsx. Font size `clamp(360px, 90vw, 1100px)`. Config in localStorage, "番茄钟" tab in SettingsPage.
- **LAN access fix (v1.5.56)**: Dev mode hardcoded `localhost:3001` → relative URLs + Vite WS proxy.
- **Dashboard + panels (v1.5.53–55)**: `LeftPanel` vertical tabs (FileTree + GitPanel). Card drag-and-drop ordering. Project status derived from in-memory `terminals` Map (v1.5.70) instead of disk reads.
- **Notifications (v1.5.49→65)**: `NotifyService` + browser push + webhook POST. LAN HTTP falls back to toast. `lib/notify.ts` navigation + `TerminalSearch` Ctrl+F overlay.
- **Removed features**: Chat SDK mode (v1.5.46), ambient sound (v1.5.54), chat mode tab (v1.5.61), Stop button (v1.5.62), built-in shortcuts (v1.5.52). JSONL history subscription kept for right panel.
- **Fix: initProjectTerminal --continue (v1.5.58)**: Always pass `continueSession=true` — safe because existing terminals return early.
- **Fix: image preview cache (v1.5.63)**: `&t=Date.now()` cache-busting on `getRawFileUrl()`.
