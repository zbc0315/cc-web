...
**当前版本**: v2026.5.18-j ｜ **包名**: `@tom2012/cc-web` ｜ **MIT** ｜ https://github.com/zbc0315/cc-web
...
项目：`@tom2012/cc-web`。当前版本 **v2026.5.18-j**（2026-05-18 发布，npm registry latest，LLM 节点完成判定从"outputs 字段变化"改为"LLM 自报 done flag"：新增 sidecar `.ccweb/tracks/<basename>.run-state.json` 记录运行进度；LLM 通过 Edit/Write 工具改文件中 `nodes.<id>.done=true` 完成 / `failed=true+reason` 失败；prompt 系统指令明确告知 LLM 多步交互后再标 done）。daemon 是否已升级 = 用户当前消息明示，过去会话授权不传递。
