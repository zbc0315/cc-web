...
- **Revert Pomodoro countdown size (v1.5.77)**: `PomodoroOverlay` font size reverted from `clamp(180px, 45vw, 550px)` back to `clamp(360px, 90vw, 1100px)` (v1.5.72 size). The v1.5.73 halving was too small.
- **Voice-to-text input + floating PluginDock capsule (v1.5.86)**: (1) `TerminalDraftInput` gains a microphone button (Mic icon from lucide-react) between StopCircle and Send buttons, using Web Speech API (`SpeechRecognition`/`webkitSpeechRecognition`). Click toggles recording on/off; long-press spacebar (>300ms hold) starts recording, release stops — space character typed during hold is cleaned up. `SpeechRecognitionCompat` interface provides cross-browser types without depending on DOM lib definitions. Recording state shown via red `animate-pulse` on mic button. Transcribed text appended to textarea value. Graceful degradation: button hidden when browser lacks Speech API. Language: `zh-CN`. (2) `PluginDock` redesigned from full-width border-bottom bar to a floating semi-transparent capsule: `fixed top-14 left-1/2 -translate-x-1/2`, `rounded-full bg-background/60 backdrop-blur-md border-border/50 shadow-lg`. Plugin buttons changed from text labels to `w-7 h-7` circular icons showing name initials (first 2 chars). Active state: `bg-blue-500/20` + animated blue dot indicator below icon via `AnimatePresence`. Removed `Puzzle` icon prefix. Hover scale 1.12x, tap 0.92x.
- **Plan-Control INIT_MD template anti-pattern guide (v1.5.85)**: `templates.ts` INIT_MD expanded with 5 ✅/❌ contrast rules targeting real AI mistakes observed during manual testing: (1) `task` is a single-line instruction not a section header — no indented sub-content; (2) `call` only accepts literal function names — no dynamic dispatch via `call ${var}`; (3) sequential tasks don't need list+loop — just write task lines; (4) no lengthy Markdown docs in main.pc — it's an execution blueprint; (5) `for` is for same-operation × multiple-data only. Also added DSL feedback channel: AI writes limitations to `.plan-control/dsl-feedback.md`.
- **Plan-Control deterministic task orchestration (v1.5.84)**: Custom DSL ("pc language") for deterministic AI task orchestration. Backend: `backend/src/plan-control/` — `types.ts` (ASTNode, PlanState, NodeRecord, PlanTreeNode, PlanConfig), `parser.ts` (regex line parser + indentation-based AST builder with Chinese+English identifier support, variable interpolation `${var}`), `checker.ts` (semantic validation: predecessor-task rule, break/continue/return scope, undefined func, DFS recursion detection), `templates.ts` (init/output/spec markdown templates), `executor.ts` (~750 lines, EventEmitter-based state machine IDLE→SENDING→WAITING→PROCESSING with REPLANNING/PAUSED/STOPPED/COMPLETED states, file-based AI communication via `.plan-control/nodes/` JSON skeletons, fs.watch+polling detection, exponential backoff nudge, crash recovery via state.json, variable scoping with call frame local_vars). `routes/plan-control.ts` REST API (init/check/start/pause/resume/stop/nodes/tree). Frontend: `PlanPanel.tsx` (adaptive toolbar, WS-driven tree patching), `TaskTree.tsx` (SVG topology graph with zoom/pan, 6-state coloring, running pulse animation, node detail popover). WS events: `plan_status`/`plan_node_update`/`plan_nudge`/`plan_replan` flow from executor → index.ts broadcastToPlanClients → TerminalView callbacks → ProjectPage lifted state → LeftPanel → PlanPanel. PlanPanel lazy-loaded via `React.lazy` named export wrapper. Key executor fixes: `advanceLine()` handles blank-line gaps in flatNodes (finds next valid node instead of jumping to end); `checkCallEnd()` implicit function return when execution leaves func body scope.
- **Fix: plugin iframe blocked by X-Frame-Options DENY (v1.5.83)**: Security headers middleware set `X-Frame-Options: DENY`, blocking all iframe embedding — including same-origin plugin FloatWindows (Pomodoro etc.). Changed to `SAMEORIGIN` so plugin iframes load normally while external sites still cannot embed ccweb. Also added `frame-src 'self'` to CSP header.
- **Fix: xterm.js requestMode ReferenceError in production build (v1.5.82)**: `@xterm/xterm@6.0.0` uses a TypeScript `const enum` compiled to an IIFE pattern `let r;(P=>...)(r||={})`. Vite's esbuild minifier incorrectly removes the `let r;` declaration (sees it as write-only/dead code) while keeping the `r={}` reference, causing `ReferenceError: r is not defined` at runtime when the terminal receives a DECRQM escape sequence. Fix: switched production minifier from esbuild to terser via `build.minify: 'terser'` in `vite.config.ts`. Terser correctly preserves the declaration. Added `terser@^5.46.1` as frontend dependency.
- **Fix: opencode adapter infinite restart loop (v1.5.81)**: `OpencodeAdapter.buildCommand()` returned `'opencode --yolo'` but `--yolo` is not a valid opencode CLI flag — opencode printed help and exited immediately, triggering terminal-manager's auto-restart in an infinite loop. Fix: removed `--yolo` flag entirely; `buildCommand()` now always returns `'opencode'`. opencode's default command starts its TUI; permission control is managed via opencode's own config, not CLI flags.
- **Fix: LAN file download broken — raw/json file instead of actual file (v1.5.80)**: `FileTree.handleDownload` replaced `<a>` click approach with `fetch` → `Blob` → `createObjectURL` pattern. Previous approach had three bugs: (1) `<a>` element not in DOM — some browsers (Firefox) ignored `.click()` and `download` attribute, causing file to save as "raw" (from URL path); (2) error responses (401/403/404 JSON) were downloaded as files instead of shown as errors; (3) no `Content-Disposition` header meant browsers displayed known MIME types inline. Fix: `fetch()` checks `res.ok` first — errors shown via `toast.error()` with parsed JSON message; success creates `Blob` URL for download with correct filename. Backend `/raw` endpoint gains `?dl=1` param support — sets `Content-Disposition: attachment; filename="..."` header for explicit download requests.
- **Fix: WSL2 missing config.json causes wrong workspace path (v1.5.79)**: When `~/.ccweb/config.json` doesn't exist (user hasn't run `ccweb setup`), localhost auth fell back to username `'local'` which wasn't admin — `getUserWorkspace('local')` returned `~/Projectslocal` (doesn't exist) instead of `~/Projects`, causing "Path not found" in filesystem browser. Fix: `auth.ts` fallback changed from `'local'` to sentinel `'__local_admin__'`; `config.ts` `isAdminUser()` recognizes this sentinel when `getAdminUsername()` returns `undefined` (no config), treating localhost as admin. `generateLocalToken()` also gains try-catch fallback with a temporary JWT secret for the no-config case. `filesystem.ts` auto-creates workspace directory when it matches defaultPath and doesn't exist.
- **CliToolAdapter pattern + multi-CLI support + usage monitor + notifications fix (v1.5.78)**: (1) **CliToolAdapter strategy pattern**: `backend/src/adapters/` — `CliToolAdapter` interface abstracts CLI-tool-specific behavior (command building, session reading, hooks, model/skills/usage). Registry `getAdapter(tool)` returns singleton per tool. Four adapters: `ClaudeAdapter` (full implementation extracted from session-manager/hooks-manager/routes/claude), `OpencodeAdapter` (`opencode --yolo`, no session/hooks), `CodexAdapter` (`codex --ask-for-approval never --sandbox danger-full-access`, no session/hooks), `QwenAdapter` (`qwen-code --yolo`, no session/hooks). Callers handle `null` returns gracefully. `terminal-manager.ts` uses `adapter.buildCommand()` + `adapter.supportsContinue()`. `session-manager.ts` delegates `parseLine()`/`parseLineBlocks()` to adapter, passes `cliTool` through `WatchState`. `hooks-manager.ts` iterates all adapters for install/uninstall (only Claude has hooks). `routes/claude.ts` generalized with `?tool=` query param for model/skills/usage. `/api/tool` route alias added. (2) **Windows platform gate**: `bin/ccweb.js` and `backend/src/index.ts` reject `process.platform === 'win32'` at startup with WSL2 install link. WSL reports `'linux'` so unaffected. (3) **Usage monitor settings**: New "用量监控" tab in SettingsPage — radio select which CLI tool to monitor (stored in `STORAGE_KEYS.usageMonitorTool`). `UsageBadge` reads tool selection, passes `?tool=` to `/api/projects/usage`. Custom event `ccweb:usage-tool-change` triggers same-tab re-fetch; `StorageEvent` handles cross-tab. Adapter `queryUsage()`/`clearUsageCache()` interface — only Claude has implementation (OAuth API), others return `{}`. (4) **Notifications fix**: `project_stopped` WS message now broadcast to both `/ws/dashboard` clients AND `/ws/projects/:id` clients — fixes ProjectPage not receiving notifications. Notification logic extracted to `lib/notify.ts`: `requireInteraction: true` (stays until clicked), `onclick` navigates to `/projects/:id` + `close()`. Toast fallback uses `duration: Infinity` + "查看" action button. Notification permission request moved from DashboardPage to App.tsx (global). `useProjectWebSocket` gains `onProjectStopped` callback. (5) **Frontend dynamic model/skills**: `TerminalDraftInput` accepts `cliTool` prop, fetches models via `getToolModels(tool)` and skills via `getToolSkills(tool)`. Skills button and model button hidden when adapter returns empty data. `ModelPanel` accepts dynamic models list instead of hardcoded `MODEL_LIST`. (6) **OpenCode adapter fix**: removed incorrect Claude-specific flags (`--dangerously-skip-permissions`/`--continue`), changed to `--yolo`, `supportsContinue()` returns `false`. (7) **Auto-restart message fix**: `handleExit()` in `terminal-manager.ts` checks `adapter.supportsContinue()` — displays "restarting with --continue" only for tools that support it.
- **WSL/Windows compatibility fixes (v1.5.76)**: (1) `terminal-manager.ts`: shell fallback changed from hardcoded `/bin/zsh` to `fs.existsSync('/bin/zsh') ? '/bin/zsh' : '/bin/bash'` — WSL default installs lack zsh, causing all terminal spawns to fail. (2) `routes/plugins.ts`: replaced `execSync('unzip ...')` with `adm-zip` (pure JS, added as dependency) — WSL minimal installs lack `unzip`. (3) `bin/ccweb-plugin.js`: replaced `execSync('zip ...')` with Node.js `zlib.deflateRawSync` inline ZIP builder — no external `zip` command needed. Generated `build.js` also uses pure JS ZIP. (4) `bin/ccweb.js` `openBrowser()`: detects WSL via `/proc/version` containing `microsoft`, tries `wslview` → `sensible-browser` → `xdg-open` fallback chain. (5) `bin/ccweb.js` daemon: `fork()` gains `detached: true` (daemon only) so child process survives terminal close in WSL. `stopServer()` kills process group (`-pid`) with 5s wait + SIGKILL fallback. `updatePackage()` also uses process group kill. (6) `routes/filesystem.ts`: multer temp dir changed from hardcoded `/tmp/ccweb-uploads` to `path.join(os.tmpdir(), 'ccweb-uploads')`.
- **Plugin system with iframe sandbox + bundled Pomodoro (v1.5.75)**: Full plugin architecture. Backend: `plugin-manager.ts` (lifecycle: loadAll/install/uninstall/updateUserConfig/setEnabled, stores plugins in `~/.ccweb/plugins/`, registry in `plugin-registry.json`, data in `plugin-data/`); `routes/plugins.ts` (REST: list/install/uninstall/update/config/enabled/data); `routes/plugin-bridge.ts` (permission-gated bridge: project:status/list, terminal:send, session:read, system:info, storage — uses `x-plugin-id` header + `requirePermission()` middleware); `routes/skillhub.ts` gains `GET /plugins` endpoint (5-min cache, fetches `plugins.json` from GitHub). Frontend: `FloatWindow.tsx` (draggable/resizable iframe `sandbox="allow-scripts allow-same-origin"`, postMessage bridge handler routing SDK calls to REST, z-index raise on click, disable iframe pointer-events during drag/resize); `FloatManager.tsx` (AnimatePresence renderer); `PluginDock.tsx` (persistent bar below header, enabled plugin buttons with active dot, auto-refetch on window focus); `PluginLayer` in App.tsx manages `allPlugins` + `openIds` (persisted to `cc_plugin_open` localStorage), scope filtering via `derivePageContext(pathname)` — `global`/`dashboard`/`project`/`project:specific`. Plugin SDK: `plugin-sdk/ccweb-plugin-sdk.js` — IIFE setting `window.ccweb.*` (getSystemInfo/getProjectList/getProjectStatus/sendTerminal/getSessionHistory/getStorage/setStorage/backendApi) using postMessage RPC with callId-based async and 10s timeout. CLI scaffold: `bin/ccweb-plugin.js` — `ccweb-plugin create <name>` (interactive: name/author/description/backend/scope/permissions/size, generates manifest.json + frontend/index.html + optional backend/index.js + build.js) and `ccweb-plugin build` (zip). SkillHubPage gains "插件" tab showing installed plugins (enable/disable/uninstall) and hub plugins (install). `manifest.json` schema: id/name/version/author/description/type(float)/float(defaultWidth/Height/minWidth/Height/resizable/scope.allowed+default/clickable.allowed+default)/permissions/frontend.entry/backend?.entry. User config validated against manifest constraints (scope.allowed, clickable.allowed). `installBundled()` auto-installs plugins from package's `plugins/` dir on first run (version check prevents re-install). Bundled `plugins/pomodoro/` — 番茄钟 timer (work/break phases, configurable durations via plugin storage, browser notifications). npm package gains `plugins/`, `plugin-sdk/` in files array, `ccweb-plugin` in bin.
- **Draggable float input + draftMode persistence (v1.5.74)**: (1) Float input box is now draggable — `mousedown` on toolbar starts drag (textarea/buttons excluded), `mousemove` updates DOM directly (zero re-render), `mouseup` commits position to state + localStorage. Toolbar shows `cursor-grab`/`cursor-grabbing`. Coordinates clamped to viewport bounds. (2) `draftMode` (float/bottom/hidden) and float position (x/y) persisted per project in localStorage under `STORAGE_KEYS.draftState(id)` as `{ mode, floatX, floatY }`. `TerminalView` reads on mount and restores last state. Ctrl+I mode switch also writes to storage. (3) `TerminalDraftInput` gains `floatPosition`/`onFloatPositionChange` props; float mode position changed from hardcoded CSS (`bottom-[20vh]`, `left: 25vw`) to dynamic `left`/`top` style from stored position. Default position: `x = 25vw, y = 80vh - 200px`.
- **Pomodoro countdown size halved (v1.5.73)**: `PomodoroOverlay` countdown font size reduced from `clamp(360px, 90vw, 1100px)` to `clamp(180px, 45vw, 550px)` — all three clamp values halved.
- **Float input default + Pomodoro size (v1.5.72)**: (1) `TerminalView` default `draftMode` changed from `'bottom'` to `'float'` — input box appears as floating capsule by default. Ctrl+I cycle order updated to `float → hidden → bottom → float`. (2) Pomodoro overlay countdown font size scaled 5×: `clamp(72px, 18vw, 220px)` → `clamp(360px, 90vw, 1100px)`. (3) Simplify cleanup from v1.5.71: MCP type normalized to `ClaudeSkillItem` (backend emits `command` not `name`; `ClaudeSkillsData.mcp` is now `ClaudeSkillItem[]`; frontend `.map()` removed); `ModelPanel` active check changed from `displayModelName()` string comparison to `normalized.includes(key)`; double-click race condition in `handleToggleSkills` fixed with `skillsLoadingRef`; decorative section separator comments removed.
