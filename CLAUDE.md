...
- **Chat SDK mode removed (v1.5.46)**: `ChatProcessManager`, `ChatInputBar`, `mode` field, `switch-mode` endpoint all deleted due to 5 architectural conflicts. The JSONL history tab (`ChatView` + `chat_message` WS) is **kept** — reads Claude's native session files, no subprocess.
- **React.lazy named export pattern**: `ChatView` has only a named export. The lazy import requires `.then((m) => ({ default: m.ChatView }))` — do not remove this wrapper.
...
- **v1.5.90**: Memory Pool（楔形容器与浮球系统）— 项目级 AI 记忆管理。左面板新增"记忆"Tab，初始化创建 `.memory-pool/`（SPEC.md/QUICK-REF.md/state.json/index.json），自动追加 CLAUDE.md 指令让 AI 自动读取/操作记忆池。浮力公式驱动优先级排序（B₀+α·H衰减），列表视图（活跃层/深层分区）+ SVG 浮球全景弹窗，四个快捷按钮（整理/读取/保存/通用）发送预设指令到终端。
...
- **v1.5.84–85**: Plan-Control DSL — custom "pc language" for deterministic task orchestration. Parser/checker/executor state machine, `PlanPanel` + `TaskTree` SVG topology.
- **v1.5.78–83**: CliToolAdapter (multi-CLI: Claude/OpenCode/Codex/Qwen). Fixes: xterm.js prod ReferenceError (→terser), X-Frame-Options SAMEORIGIN, opencode `--yolo` removal, LAN file download, WSL2 `__local_admin__` sentinel.
- **v1.5.75–77**: Plugin system (iframe sandbox, SDK, CLI scaffold, Pomodoro). WSL/Windows compat (shell fallback, adm-zip, detached daemon).
- **v1.5.59–74**: Terminal draft input (3-state mode, Skills panel, Model dropdown, resizable panels). Non-modal floating dialogs. Pomodoro timer. Dashboard panels + card drag ordering.
- **v1.5.48–58**: Tags, todo board, session share (v1.5.51). Session search, Git panel, mobile layout (v1.5.50). Notifications + terminal search (v1.5.49). Security hardening (v1.5.48). LeftPanel vertical tabs. LAN access fix.
